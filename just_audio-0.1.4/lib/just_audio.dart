...
class AudioPlayer {
  ...
  final _playerVolumeSubject = BehaviorSubject<double>();
  ...
  // IconData _mycon = Icons.volume_up;
  ...
  AudioPlayer._internal(this._id) : _channel = _init(_id) {
    ...
    _playerVolumeSubject
        .addStream(playbackEventStream.map((state) => state.volume).distinct());
  }
  ...
  Stream<double> get playerVolumeStream => _playerVolumeSubject.stream;
  ...
  // Stream<double> get getVol =>

  // IconData get mycon => _mycon;
  ...
  Future<void> play() async {
    ...
    int res = await _invokeMethod('play');
...
// }
  ...
  Future<void> setVolume(final double vol) async {
    _volume = vol;
    
    // _playerVolumeSubject.add(_volume);
    await _invokeMethod('setVolume', [vol]);
  }
  ...
  Future<void> seek() async {
    await _invokeMethod('seek', [100000]);
  }
